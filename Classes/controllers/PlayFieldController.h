...
//PlayFieldController 是桌面区（PlayFieldView）的控制器：
//负责把 GameModel 中的“桌面牌（table）”渲染到 PlayFieldView；
//处理点击桌面牌的事件；在满足规则时，把桌面牌移动到托盘顶部位，并记录** 撤销（Undo）** 信息，
//同时更新 GameModel（table、top 等字段）。

class PlayFieldView;// 桌面区视图（上半区）
class GameView;// 顶层视图容器（提供 stack/playfield 访问 & 工具方法）
